...
When the component catalog is empty, use **Initialize architecture model** in the logical component graph. Blueprint derives one repository-level starter component from the current package/project identity, published file boundary, and `DESIGN.md`, shows its ID, kind, and owned paths for confirmation, and applies it through the same hash-bound architecture preview workflow. Initialization never overwrites or appends to a non-empty catalog and intentionally creates a coarse starting boundary rather than pretending to infer every internal component.
...
After Architecture design is opened once, switching to another Blueprint tab hides that workspace without unmounting it. The active Session subscription, streaming turn, proposal-card state, scroll position, and unsent draft therefore remain intact when the developer returns. The unsent draft and exact archived backing Session id are additionally persisted under the current project, architecture protocol, and selected Feature/component focus. A remount restores those values and tries the exact Session before title-based discovery; if the old binding is unavailable, cached visible history remains while the next submission creates a replacement backing Session.

The assistant first produces a structured before/after proposal. When the result is exactly one current-schema component upsert or deletion, the finalized reply becomes an inline proposal card rather than a list of manual file instructions. Select **Generate change preview** to ask the Host for the exact component, typed-edge, deployment, contract, document, and approval-invalidation delta. Only after that preview exists does **Confirm and apply** appear; it applies the same change with the exact preview SHA-256 and refreshes the graph. A stale preview or invalid reference fails closed.

This card does not invent a Component tab or Relationships panel. It supports one registered component change at a time and only the current kinds, statuses, string contract IDs, document declarations, and component-to-component relation types. Feature parent edits, Feature-to-Feature relations, structured contract direction/purpose metadata, multi-component transactions, Product brief changes, and proposed Spec changes remain discussion or a separate governed workflow. The architecture Session cannot write by itself, edit implementation files, move lifecycle documents to `implemented`, write approval records, or approve its own result.
