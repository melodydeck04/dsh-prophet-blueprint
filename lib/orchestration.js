...
import { coordinatorMessage, parseNaturalDispatch, skillRoutingMessage } from "./chat-commands.js";
...
function actionContext(feature, action) {
	if (!feature.workflow.context) return null; // compatibility for injected test/legacy dashboard providers
	const context = feature.workflow.context;
	if (!context.allowedActions.includes(action)) {
		throw new Error(`action '${action}' is not allowed in stage '${context.currentStage}'; next required action: ${context.nextRequiredAction ?? "none"}`);
	}
	return context;
}
...
export function createBlueprintOrchestrator(_ctx, dependencies = {}) {
	...
	async function dispatchCommand(commandName, invocation) {
		...
		invocation.agent.steer(userMessage(skillRoutingMessage({ request: invocation.rawInput, features: dashboard.catalog.features }), "routing"));
		return { kind: "success", text: "已读取项目上下文，继续在当前 Chat 处理请求。" };
	}
	...
	async function begin(args, agent) {
		...
		const next = await dashboardFor(agent);
		return { action: "begin", featureId: feature.id, stage: "implementing", cycleId: started.record?.cycle?.id ?? null, workflowContext: exactFeature(next, feature.id).workflow.context ?? null, guidance: "审批已绑定；请在当前 DSH Chat 中按 Spec Scope 实现，并在完成相关检查后调用 blueprint_dispatch action=complete。" };
	}
	...
	async function complete(args, agent) {
		...
		actionContext(feature, "complete");
	...
	async function dispatchNatural(args, execution) {
		...
		const action = args.action ?? "route";
		if (action === "status") {
			const result = await status({ agent });
			return { action, workflowContext: result.context ?? null, guidance: result.text };
		}
		if (action === "route") {
			const dashboard = await dashboardFor(agent);
			return { action, guidance: skillRoutingMessage({ request: args.request, features: dashboard.catalog.features }) };
		}
		if (!["refine", "begin", "complete"].includes(action)) throw new Error(`Unknown Blueprint action '${action}'`);
		if (action === "begin") return begin(args, agent);
		if (action === "complete") return complete(args, agent);
		const dashboard = await dashboardFor(agent);
		if (args.featureId) actionContext(exactFeature(dashboard, args.featureId), "refine");
		...
		scheduleAutoCompactAfter({ agent, signal: execution?.signal });
	...
	async function status(invocation) {
		const dashboard = await dashboardFor(invocation.agent);
		const rows = dashboard.catalog.features.map((feature) => `- ${feature.id}: ${feature.workflow.stage} · next: ${feature.workflow.context?.nextRequiredAction ?? "status"}`).join("\n");
		return { kind: "success", context: dashboard.catalog.features[0]?.workflow.context ?? null, text: `Blueprint · ${dashboard.project.name}\n${rows || "No Features"}` };
	}
	...
	function dispatchToolDefinition() {
		return {
			name: DISPATCH_TOOL,
			description: "Use route for read-only request context and native Skill selection, status for existing Feature stages, refine for a requested change, begin for exact approved delivery, complete for executed verification evidence. Missing capabilities or invalid authority produce errors; never invent results.",
			parameters: {
				type: "object",
				properties: {
					action: { type: "string", enum: ["route", "status", "refine", "begin", "complete"] },
