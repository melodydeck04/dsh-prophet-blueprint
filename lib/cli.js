...
import { resolveTodoPath, loadTodosForSpec, featureIdForSpecPath } from "./spec-todos.js";
...
import { requestFeatureVerification, loadVerificationCatalog, validateVerificationPayload } from "./verification.js";
...
Usage:
  ...
  design-blueprint verification status <feature-id> [--cwd <path>] [--json]
  design-blueprint verification dry-run <feature-id> --payload-file <path> [--cwd <path>] [--json]
...
verification status reads the verification record (cycle, stage, attempts, history) without writing.
verification dry-run validates a payload file without writing.
...
async function runVerification(flags, positional) {
	...
	const action = positional[0];
	if (action === "request") {
		if (positional.length !== 2) throw new Error("verification request requires 'request <feature-id>'");
	...
	if (action === "status") {
		if (positional.length !== 2) throw new Error("verification status requires 'status <feature-id>'");
		const featureId = positional[1];
		const projectRoot = await resolveBlueprintRoot(cwd);
		const snapshot = await workingTreeSnapshot(projectRoot);
		const configResult = await loadConfig(snapshot);
		const verificationFilePath = `${configResult.config.features.verificationsRoot}/${featureId}.json`;
		if (!snapshot.exists(verificationFilePath)) {
			process.stderr.write(`error: verification record not found for feature '${featureId}'\n`);
			return 1;
		}
		const text = await snapshot.readText(verificationFilePath);
		const record = JSON.parse(text);
		if (flags.json) {
			process.stdout.write(JSON.stringify(record, null, 2) + "\n");
			return 0;
		}
		process.stdout.write(`Feature: ${featureId}\n`);
		process.stdout.write(`Stage: ${record.stage}\n`);
		if (record.snapshot) process.stdout.write(`Snapshot: ${record.snapshot.kind}:${record.snapshot.digest}\n`);
		if (record.spec) process.stdout.write(`Approved-hash: ${record.spec.approvedHash}\n`);
		process.stdout.write(`Attempts: ${(record.attempts ?? []).length}\n`);
		for (const attempt of record.attempts ?? []) {
			process.stdout.write(`  - ${attempt.id}: ${attempt.conclusion ?? "in-progress"} (session=${attempt.sessionId ?? "?"}) ${attempt.summary ?? ""}\n`);
		}
		return 0;
	}
	if (action === "dry-run") {
		if (positional.length !== 2) throw new Error("verification dry-run requires 'dry-run <feature-id>'");
		if (typeof flags["payload-file"] !== "string") throw new Error("verification dry-run requires --payload-file <path>");
		const featureId = positional[1];
		const projectRoot = await resolveBlueprintRoot(cwd);
		const snapshot = await workingTreeSnapshot(projectRoot);
		const configResult = await loadConfig(snapshot);
		...
		const record = JSON.parse(await snapshot.readText(verificationFilePath));
		const specSourceFile = record.spec?.sourceFile;
		if (!specSourceFile || !snapshot.exists(specSourceFile)) {
			process.stderr.write(`error: Spec file '${specSourceFile ?? "(none)"}' is not in the snapshot\n`);
			return 1;
		}
		const specContent = await snapshot.readText(specSourceFile);
		const specsResult = await loadSpecs(snapshot, configResult.config);
		const spec = specsResult.specs.find((entry) => entry.file === specSourceFile);
		if (!spec) {
			process.stderr.write(`error: Spec '${specSourceFile}' could not be parsed\n`);
			return 1;
		}
		const payloadText = await readFile(flags["payload-file"], "utf8");
		const payload = JSON.parse(payloadText);
		const specChangePackage = {
			acceptance: new Map((spec.acceptance ?? []).map((entry) => [entry.id, entry.oracle])),
			changePackage: { verification: { targets: spec.verification?.targets ?? [] } },
		};
		const result = validateVerificationPayload(specChangePackage, payload);
		if (flags.json) {
			process.stdout.write(JSON.stringify(result, null, 2) + "\n");
			return result.ok ? 0 : 1;
		}
		if (result.ok) {
			process.stdout.write("OK: payload is valid\n");
			return 0;
		}
		process.stdout.write(`FAIL: ${result.issues.length} issue(s) found\n`);
		for (const [index, line] of result.issues.entries()) {
			process.stdout.write(`  ${index + 1}. ${line}\n`);
		}
		return 1;
	}
	throw new Error("verification requires one of: 'request <feature-id>', 'status <feature-id>', 'dry-run <feature-id> --payload-file <path>'");
}
...
async function runTodo(flags, positional) {
	...
	if (status === "done") {
		...
		await emitAutoCompact(cwd, sessionId, specRelative, time);
	}
	return 0;
}
...
async function emitAutoCompact(cwd, sessionId, currentTaskSpecPath, nowMs) {
	const { maybeAutoCompact, _resetDispatchWarning } = await import("./todo-compact-trigger.js");
	_resetDispatchWarning();
	const currentFeatureId = featureIdForSpecPath(currentTaskSpecPath, cwd);
	...
	const bytes = result.bytesSincePreviousTaskDone ?? 0;
	const humanMiB = (bytes / (1024 * 1024)).toFixed(2);
	switch (result.reason) {
		case "feature-switch":
			process.stdout.write(`Auto-compact: fired (feature-switch: ${result.previousFeatureId ?? "?"} -> ${currentFeatureId ?? "?"}, ${humanMiB} MiB accumulated).\n`);
			break;
		case "same-feature":
			process.stdout.write(`Auto-compact: skipped (same feature as previous: ${currentFeatureId ?? "?"}).\n`);
			break;
		case "under-threshold":
			process.stdout.write(`Auto-compact: skipped (under threshold: ${bytes} bytes < 200 KiB).\n`);
			break;
		case "no-previous-task":
			process.stdout.write(`Auto-compact: skipped (no previous task).\n`);
			break;
		case "no-dispatch-surface":
			process.stdout.write(`Auto-compact: skipped (no DSH slash-command dispatch surface; manual /compact required).\n`);
			break;
		case "dispatch-failed":
			process.stdout.write(`Auto-compact: failed (${result.error ?? "unknown error"}).\n`);
			break;
		default:
			process.stdout.write(`Auto-compact: skipped (${result.reason}).\n`);
	}
}
