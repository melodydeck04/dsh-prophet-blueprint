...
function computeArchitectureDelta(state, parsedComponent, deletedId) {
	...
	if (parsedComponent) {
		...
		approvalInvalidations.push({ kind: "component-change", component: parsedComponent.id, file: `${state.config.features.approvalsRoot}/spec-governance--architecture-design.json` });
	}
	if (deletedId) {
		...
		if (previous) {
			...
			approvalInvalidations.push({ kind: "component-deleted", component: previous.id, file: `${state.config.features.approvalsRoot}/spec-governance--architecture-design.json` });
		}
	}
	return {
		added: [...new Set(added)],
