...
export async function gitRoot(cwd) {
	try {
		const { stdout } = await execFile("git", gitArguments(cwd, "rev-parse", "--show-toplevel"), { windowsHide: true });
		return stdout.trim();
	} catch {
		return null;
	}
...
export async function stagedChanges(root) {
	const { stdout } = await execFile(
		"git",
		gitArguments(root, "diff", "--cached", "--name-status", "-z", "--find-renames", "--find-copies"),
	...
	return {
		...
		async readBytes(path) {
			...
			try {
				const info = await stat(join(root, ...normalized.split("/")));
				if (!info.isFile()) return null;
				return await readFile(join(root, ...normalized.split("/")));
			} catch {
				return null;
			}
		},
...
export async function gitIndexSnapshot(root) {
	const { stdout } = await execFile("git", gitArguments(root, "ls-files", "-z", "--cached"), {
		windowsHide: true,
	...
	return {
		...
		async readText(path) {
			...
			if (!fileSet.has(normalized)) return null;
			try {
				const { stdout: content } = await execFile("git", gitArguments(root, "show", `:${normalized}`), {
					windowsHide: true,
					maxBuffer: 16 * 1024 * 1024,
				});
				return content;
