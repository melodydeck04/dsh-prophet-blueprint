...
export const DEFAULT_DECOMPOSITION = Object.freeze({ maxReq: 8, maxScopePaths: 5, maxLines: 1500 });
...
function optionalPathArray(value, field, issues) {
	if (value === undefined) return [];
	if (!Array.isArray(value) || value.some((entry) => typeof entry !== "string" || entry.length === 0)) {
		issues.push(issue(`${field} must be a string array`, `Fix ${field} in ${CONFIG_FILE}`));
		return [];
	}
...
export async function loadConfig(snapshot) {
	...
	const completionHygiene = raw.completionHygiene ?? {};
	let specsRoot = ".specs";
	try {
	...
	const config = {
		...
		completionHygiene: {
			secretAllow: optionalPathArray(completionHygiene.secretAllow, "completionHygiene.secretAllow", issues),
			temporaryAllow: optionalPathArray(completionHygiene.temporaryAllow, "completionHygiene.temporaryAllow", issues),
		},
		documentation: structuredClone(DEFAULT_CONFIG.documentation),
	};
	const rawDecomposition = raw.decomposition ?? {};
	if (typeof rawDecomposition === "object" && rawDecomposition !== null && !Array.isArray(rawDecomposition)) {
		const thresholds = { ...DEFAULT_DECOMPOSITION };
		if (typeof rawDecomposition.maxReq === "number") thresholds.maxReq = rawDecomposition.maxReq;
		if (typeof rawDecomposition.maxScopePaths === "number") thresholds.maxScopePaths = rawDecomposition.maxScopePaths;
		if (typeof rawDecomposition.maxLines === "number") thresholds.maxLines = rawDecomposition.maxLines;
