...
import { approveFeatureProposal, loadFeatureWorkflow, publicWorkflowStage, workflowContext } from "./workflow.js";
...
export async function handleBlueprintAction(input) {
	...
	if (input.action === "approve-and-begin") {
		if (typeof input.featureId !== "string" || typeof input.expectedSpecHash !== "string" || typeof context.sessionId !== "string") {
			throw new BlueprintApiError(400, "INVALID_APPROVAL_CONTINUATION", "featureId, expectedSpecHash, and sessionId are required");
		}
		const before = await dashboardForInput(input);
		const prior = before.catalog.features.find((feature) => feature.id === input.featureId);
		if (!prior) throw new BlueprintApiError(404, "UNKNOWN_FEATURE", `Feature '${input.featureId}' does not exist`);
		try {
			await approveFeatureProposal({ cwd: await effectiveCwd(input), featureId: input.featureId, expectedSpecHash: input.expectedSpecHash });
			const begun = await beginFeatureImplementation({
				cwd: await effectiveCwd(input),
				featureId: input.featureId,
				expectedSpecHash: input.expectedSpecHash,
				intent: "natural",
				requestSessionId: context.sessionId,
			});
			const dashboard = await dashboardForInput(input);
			return {
				receipt: {
					featureId: input.featureId,
					specHash: input.expectedSpecHash,
					cycleId: begun.record?.cycle?.id ?? null,
					continuationRequired: prior.workflow.stage !== "implementing",
					chatDelivery: { status: prior.workflow.stage === "implementing" ? "already-active" : "pending" },
				},
				dashboard,
			};
		} catch (error) {
			throw new BlueprintApiError(409, "APPROVAL_CONTINUATION_FAILED", error instanceof Error ? error.message : String(error));
		}
	}
	...
	throw new BlueprintApiError(400, "UNKNOWN_ACTION", "action must be bind, unbind, dashboard, document, discover, initialize, upgrade, approve, approve-and-begin, prepare, migration-preview, migration-apply, architecture-initialize-preview, architecture-initialize-apply, architecture-preview, architecture-apply, assistant-spec-preview, assistant-spec-apply, implementation-begin, verification-request, verification-legacy-request, verification-prepare, verification-start, verification-result, verification-finalize, verification-abandon, verification-fail, or save");
}
...
export async function handleBlueprintRequest(req, res, hooks = {}) {
	try {
		...
		const result = await handleBlueprintAction(input);
		if (input.action === "approve" && typeof hooks.afterApproval === "function") {
			await hooks.afterApproval(input);
			sendJson(res, 200, { ok: true, value: await dashboardForInput(input) });
			return;
		}
		if (input.action === "approve-and-begin" && result.receipt?.continuationRequired === true && typeof hooks.afterApprovalContinuation === "function") {
			result.receipt.workflowContext = result.dashboard.catalog.features.find((feature) => feature.id === result.receipt.featureId)?.workflow.context ?? null;
			try {
				const delivery = await hooks.afterApprovalContinuation(input, result.receipt);
				result.receipt.chatDelivery = delivery ?? { status: "delivered" };
			} catch (error) {
				result.receipt.chatDelivery = { status: "failed", message: error instanceof Error ? error.message : String(error) };
			}
		}
