...
function publicFeature(feature, workflow, artifacts, architecture) {
	...
	return {
		...
		workflow: {
			...
			verification: publicVerificationRecord(workflow?.verification ?? null),
		},
	};
}
...
export async function handleBlueprintAction(input) {
	...
	if (input.action === "document") {
		...
		return getBlueprintDocument({ cwd: input.cwd, featureId: input.featureId, file: input.file });
	}
	...
	if (input.action === "assistant-spec-preview") {
		try { return await previewAssistantSpecPatch({ cwd: input.cwd, patch: input.patch }); }
	...
	if (input.action === "assistant-spec-apply") {
		...
		try {
			const result = await applyAssistantSpecPatch({ cwd: input.cwd, patch: input.patch, expectedPreviewHash: input.expectedPreviewHash, confirmBilingual: input.confirmBilingual });
			return { applied: result, dashboard: await getBlueprintDashboard(input.cwd) };
		} catch (error) {
	...
	if (input.action === "implementation-begin") {
		try {
			await beginFeatureImplementation({ cwd: input.cwd, featureId: input.featureId, expectedSpecHash: input.expectedSpecHash });
			return getBlueprintDashboard(input.cwd);
		} catch (error) {
	...
	if (input.action === "verification-request") {
		try {
			await requestFeatureVerification({ cwd: input.cwd, featureId: input.featureId, expectedSpecHash: input.expectedSpecHash });
			return getBlueprintDashboard(input.cwd);
		} catch (error) {
	...
	if (input.action === "verification-legacy-request") {
		try {
			await requestLegacyFeatureVerification({ cwd: input.cwd, featureId: input.featureId });
			return getBlueprintDashboard(input.cwd);
		} catch (error) {
	...
	if (input.action === "verification-start") {
		try {
			const started = await startFeatureVerification({ cwd: input.cwd, featureId: input.featureId, expectedRecordHash: input.expectedRecordHash, sessionId: input.sessionId, workspacePath: input.workspacePath, preparationCapability: input.preparationCapability });
			return { started: { attempt: started.attempt, resultCapability: started.resultCapability }, dashboard: await getBlueprintDashboard(input.cwd) };
		} catch (error) {
	...
	if (input.action === "verification-prepare") {
		try {
			return await prepareFeatureVerification({ cwd: input.cwd, featureId: input.featureId, expectedRecordHash: input.expectedRecordHash });
		} catch (error) {
	...
	if (input.action === "verification-result") {
		try {
			const accepted = await submitFeatureVerificationResult({ cwd: input.cwd, featureId: input.featureId, expectedRecordHash: input.expectedRecordHash, attemptId: input.attemptId, resultCapability: input.resultCapability, result: input.result });
			return { accepted: { completed: accepted.completed, verified: accepted.verified === true, hostFailure: accepted.hostFailure === true }, dashboard: await getBlueprintDashboard(input.cwd) };
		} catch (error) {
	...
	if (input.action === "verification-finalize") {
		try {
			const accepted = await completeVerifiedFeature({ cwd: input.cwd, featureId: input.featureId, expectedRecordHash: input.expectedRecordHash });
			return { accepted: { completed: accepted.completed, hostFailure: accepted.hostFailure === true }, dashboard: await getBlueprintDashboard(input.cwd) };
		} catch (error) {
	...
	if (input.action === "verification-abandon") {
		try {
			await abandonFeatureVerification({ cwd: input.cwd, featureId: input.featureId, expectedRecordHash: input.expectedRecordHash, attemptId: input.attemptId, sessionId: input.sessionId });
			return getBlueprintDashboard(input.cwd);
		} catch (error) {
	...
	if (input.action === "verification-fail") {
		try {
			const failed = await failFeatureVerificationOrchestration({ cwd: input.cwd, featureId: input.featureId, expectedRecordHash: input.expectedRecordHash, phase: input.phase, message: input.message, attemptId: input.attemptId, sessionId: input.sessionId });
			return { failed: { hostFailure: failed.hostFailure === true }, dashboard: await getBlueprintDashboard(input.cwd) };
		} catch (error) {
			throw new BlueprintApiError(409, "VERIFICATION_ORCHESTRATION_FAILED", error instanceof Error ? error.message : String(error));
		}
	}
	throw new BlueprintApiError(400, "UNKNOWN_ACTION", "action must be dashboard, document, discover, initialize, upgrade, approve, prepare, migration-preview, migration-apply, architecture-initialize-preview, architecture-initialize-apply, architecture-preview, architecture-apply, assistant-spec-preview, assistant-spec-apply, implementation-begin, verification-request, verification-legacy-request, verification-prepare, verification-start, verification-result, verification-finalize, verification-abandon, verification-fail, or save");
}
...
export async function handleBlueprintRequest(req, res, hooks = {}) {
	try {
		...
		if (input.action === "approve" && typeof hooks.afterApproval === "function") {
			...
			result = await getBlueprintDashboard(input.cwd);
		}
