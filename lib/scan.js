...
export async function scan(options = {}) {
	...
	const decompositionIssues = await evaluateDecompositionIssues(specsResult.specs, snapshot, configResult.config);
	const issues = [...configResult.issues, ...specsResult.issues, ...docsResult.issues, ...architectureResult.issues, ...verificationResult.issues, ...allocationIssues, ...lifecycleIssues, ...workflowResult.issues, ...policyIssues, ...decompositionIssues];
	return {
		cwd,
...
/** Scan every proposed Spec for size signals (REQ count, scope paths, line count) that exceed decomposition thresholds. */
async function evaluateDecompositionIssues(specs, snapshot, config) {
	const thresholds = loadDecompositionThresholds(config?.decomposition ?? null);
	const issues = [];
	for (const spec of specs) {
		if (spec.status !== "proposed") continue;
		if (!spec.file.startsWith(`${config.authority.specsRoot}/proposed/`)) continue;
		const text = await snapshot.readText(spec.file);
		if (text === null) continue;
		const result = evaluateSpecDecomposition({ file: spec.file, content: text, config: thresholds });
		if (result.ok) continue;
		const severity = result.observed.reqCount > thresholds.maxReq * 2 || result.observed.lineCount > thresholds.maxLines * 2
			? "required"
			: "recommended";
		for (const violation of result.violations) {
			issues.push({
				file: spec.file,
				check: "decomposition-contract",
				severity,
