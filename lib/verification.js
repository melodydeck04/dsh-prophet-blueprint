...
function sectionList(content, heading) {
	...
	const start = lines.findIndex((line) => {
		...
		// Split on common bilingual separators: " / ", " — ", "（", "(", "、", ",", em-dash, en-dash
		const parts = headingText.split(/\s*[\/()（）、，,—–]\s*/).filter(Boolean);
...
async function finalizeVerifiedFeature(facts, record, attempt, transactionOptions = null) {
	...
	// Synchronize the approval record with the new Spec state when finalize promotes a Spec.
	// The action list above contains content:null for spec.file and a new write for targetEn
	// (with matching .zh.md counterparts). The approval record at .blueprint/approvals/<featureId>.json
	// still references the old proposed path; without rewriting it, the snapshot at validate time
	// contains both the new implemented Spec (A-staged, eligible via its own scope) and the
	// old proposed Spec (D-staged but content-readable, eligible via the stale approval). Both
	// own the same scope, so any brief / Feature doc write hits spec-scope-ambiguity and the
	// transaction rolls back. By rewriting the approval to point at targetEn with the new
	// review hash, the validate scan sees only one eligible owner and the transaction completes.
	if (!legacy) {
		const approvalFile = posix.join(facts.config.features.approvalsRoot, `${record.featureId}.json`);
		const approvalText = await facts.snapshot.readText(approvalFile);
		if (approvalText !== null) {
			const approval = JSON.parse(approvalText);
			const newReviewHash = createHash("sha256").update(english).update("\0").update(chinese).digest("hex");
			const updatedApproval = { ...approval, spec: targetEn, specHash: newReviewHash };
			actions.push({ file: approvalFile, content: JSON.stringify(updatedApproval, null, 2) + "\n" });
		}
	}
	const completed = {
		...record,
...
export { publicRecord, sectionList, mergeCurrentBrief };
