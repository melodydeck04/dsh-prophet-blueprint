...
function parseCheck(value, index) {
	exactKeys(value, new Set(["id", "kind", "status", "summary"]), `checks[${index}]`);
	...
	return { id, kind: value.kind, status: value.status, summary: boundedText(value.summary, `checks[${index}].summary`) };
}
...
function parseAttempt(value, index) {
	exactKeys(value, new Set(["id", "sessionId", "capabilityHash", "startedAt", "completedAt", "conclusion", "summary", "acResults", "checks", "findings"]), `attempts[${index}]`);
	...
	if (value.completedAt === null && value.conclusion === null) {
		...
		return { id: value.id, sessionId: value.sessionId, capabilityHash, startedAt, completedAt: null, conclusion: null, summary: "", acResults: [], checks: [], findings: [] };
	}
	...
	return { id: value.id, sessionId: value.sessionId, capabilityHash, startedAt, completedAt: timestamp(value.completedAt, `attempts[${index}].completedAt`), ...result };
}
...
export async function startFeatureVerification({ cwd, featureId, expectedRecordHash, sessionId, workspacePath, preparationCapability }) {
	...
	const attempt = { id: `attempt-${record.attempts.length + 1}`, sessionId, capabilityHash: sha256(resultCapability), startedAt: now(), completedAt: null, conclusion: null, summary: "", acResults: [], checks: [], findings: [] };
...
function assertPassingEvidence(spec, result) {
	...
	const requiresBrowser = /real[- ]browser|browser (?:verification|scenario|workflow)|真实浏览器/i.test(spec.content);
	if (requiresBrowser && !result.checks.some((entry) => entry.kind === "browser" && entry.status === "passed")) throw new Error("the approved Spec requires passing real-browser evidence");
...
export async function submitFeatureVerificationResult({ cwd, featureId, expectedRecordHash, attemptId, resultCapability, result }) {
	if (!HASH_PATTERN.test(expectedRecordHash ?? "")) throw new Error("expectedRecordHash must be SHA-256");
	...
	const completedAttempt = { ...record.attempts[index], completedAt: now(), ...normalized };
