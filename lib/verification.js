...
export async function submitFeatureVerificationResult({ cwd, featureId, expectedRecordHash, attemptId, resultCapability, result }) {
	...
	if (suppliedCapability === null && storedCapability !== null && sha256(storedCapability) === record.attempts[index].capabilityHash) {
		// Caller did not supply a capability but the persisted one is intact; reuse it so a single-Chat
		// orchestrator that lost the in-memory copy (e.g., crashed between start and submit) can still
		// submit. This is the documented recovery path for `blueprint_dispatch action=complete`.
