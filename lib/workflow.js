...
/** Derive the finite action contract and compact turn package from durable state. */
export function workflowContext({ stage = "draft", spec = null, verification = null } = {}) {
	const currentStage = publicWorkflowStage(stage);
	const allowedActions = stage === "approved" ? ["begin"]
		: stage === "implementing" || stage === "needs_changes" ? ["complete"]
		: ["draft", "prepared", "review"].includes(stage) ? ["refine"]
		: [];
	const nextRequiredAction = currentStage === "completed" ? null
		: allowedActions[0] ?? "status";
	const changePackage = spec?.changePackage ?? {};
	return Object.freeze({
		currentStage,
		allowedActions: Object.freeze(allowedActions),
		nextRequiredAction,
		workPackage: Object.freeze({
			featureId: spec?.featureId ?? null,
			specHash: spec?.reviewHash ?? spec?.contentHash ?? null,
			scope: Object.freeze(changePackage.scope?.allowedPaths ?? []),
			tasks: Object.freeze(changePackage.execution?.tasks ?? []),
			verification: verification ? { stage: verification.stage, latestFinding: verification.attempts?.at(-1)?.findings?.at(-1) ?? null } : null,
		}),
	});
}
