window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CLIENT_VERSION = "0.14.7";
		const REVIEW_MESSAGE_MARKER = "<!-- BLUEPRINT_REVIEW_MESSAGE -->";
		const REVIEW_PROTOCOL_VERSION = "guided-v2";
		const ARCHITECTURE_PROTOCOL_VERSION = "guided-v3";
		const ARCHITECTURE_MESSAGE_MARKER = "<!-- BLUEPRINT_ARCHITECTURE_MESSAGE -->";
		const ARCHITECTURE_CHANGE_OPEN = "<blueprint-architecture-change>";
		const ARCHITECTURE_CHANGE_CLOSE = "</blueprint-architecture-change>";
...
.bp-arch-proposal{box-sizing:border-box;width:100%;margin-top:7px;border:1px solid color-mix(in srgb,var(--dsw-alias-state-business-primary,#2563eb) 32%,var(--dsw-alias-border-l2,#d1d5db));border-radius:9px;background:var(--dsw-alias-bg-layer-1,#fff);padding:10px;white-space:normal}
.bp-arch-proposal.invalid{border-color:#fecaca;background:#fef2f2}.bp-arch-proposal.applied{border-color:#bbf7d0;background:#f0fdf4}
.bp-arch-proposal-head{display:flex;align-items:flex-start;justify-content:space-between;gap:8px}.bp-arch-proposal-head strong{font-size:12px}.bp-arch-proposal-meta{margin-top:2px;color:var(--dsw-alias-label-tertiary,#6b7280);font:10px/16px var(--ds-font-family-code,monospace);overflow-wrap:anywhere}
.bp-arch-proposal-delta{margin:8px 0 0;padding:8px 10px;border-radius:7px;background:var(--dsw-alias-bg-layer-3,#f3f4f6);font-size:11px;line-height:17px}.bp-arch-proposal-delta ul{margin:4px 0 0;padding-left:18px}.bp-arch-proposal-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:9px}.bp-arch-proposal-error{margin:7px 0 0;color:#b91c1c;font-size:11px;line-height:16px}.bp-arch-proposal-success{margin:7px 0 0;color:#15803d;font-size:11px;line-height:16px}
...
- 回答保持紧凑，优先给开发者可直接确认的完整中文方案。
		...
		function validateArchitectureProposalChange(change) {
			if (change === null || typeof change !== "object" || Array.isArray(change)) return "架构变更必须是一个对象";
			if (change.action !== "upsert" && change.action !== "delete") return "action 只能是 upsert 或 delete";
			if (typeof change.id !== "string" || !/^[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$/.test(change.id)) return "id 必须是 1–64 位小写 ASCII 组件 ID";
			if (change.action === "delete") {
				return Object.keys(change).every((key) => key === "action" || key === "id") ? null : "delete 变更只能包含 action 与 id";
			}
			const fields = ["action", "id", "title", "kind", "containerId", "deployment", "status", "summary", "ownedPaths", "contracts", "dependencies", "supportedFeatures", "documents"];
			const allowed = new Set(fields);
			if (Object.keys(change).some((key) => !allowed.has(key)) || fields.some((key) => !(key in change))) return "upsert 必须完整且只能使用当前组件 Schema";
			if (typeof change.title !== "string" || typeof change.summary !== "string") return "title 与 summary 必须是字符串";
			if (!["service", "library", "plugin", "internal", "frontend", "data", "infra"].includes(change.kind)) return "kind 不受当前组件 Schema 支持";
			if (!["planned", "active", "deprecated"].includes(change.status)) return "status 不受当前组件 Schema 支持";
			if (change.containerId !== null && typeof change.containerId !== "string") return "containerId 必须是组件 ID 或 null";
			if (change.deployment !== null && typeof change.deployment !== "string") return "deployment 必须是字符串或 null";
			for (const field of ["ownedPaths", "contracts", "supportedFeatures"]) {
				if (!Array.isArray(change[field]) || change[field].some((entry) => typeof entry !== "string")) return `${field} 必须是字符串数组`;
			}
			const relations = new Set(["depends_on", "calls", "publishes", "consumes", "exposes", "extends"]);
			if (!Array.isArray(change.dependencies) || change.dependencies.some((entry) => entry === null || typeof entry !== "object" || Array.isArray(entry) || !relations.has(entry.relation) || typeof entry.target !== "string" || Object.keys(entry).some((key) => key !== "relation" && key !== "target"))) return "dependencies 含有当前组件 Schema 不支持的关系";
			if (!Array.isArray(change.documents) || change.documents.some((entry) => entry === null || typeof entry !== "object" || Array.isArray(entry) || !["required", "recommended"].includes(entry.level) || typeof entry.path !== "string" || Object.keys(entry).some((key) => key !== "level" && key !== "path"))) return "documents 必须使用 level/path 结构";
			return null;
		}

		function parseArchitectureProposal(value) {
			const source = String(value ?? "");
			const start = source.indexOf(ARCHITECTURE_CHANGE_OPEN);
			const strayClose = source.indexOf(ARCHITECTURE_CHANGE_CLOSE);
			if (start < 0) return strayClose < 0 ? { text: source, change: null, error: null } : { text: source.slice(0, strayClose).trim(), change: null, error: "架构提案包含没有开始标记的结束标记" };
			const end = source.indexOf(ARCHITECTURE_CHANGE_CLOSE, start + ARCHITECTURE_CHANGE_OPEN.length);
			if (end < 0) return { text: source.slice(0, start).trim(), change: null, error: "架构提案缺少结束标记" };
			const after = end + ARCHITECTURE_CHANGE_CLOSE.length;
			const visibleText = `${source.slice(0, start)}${source.slice(after)}`.trim();
			if (source.indexOf(ARCHITECTURE_CHANGE_OPEN, after) >= 0 || source.indexOf(ARCHITECTURE_CHANGE_CLOSE, after) >= 0) return { text: source.slice(0, start).trim(), change: null, error: "每条回复最多只能包含一个架构变更" };
			try {
				const change = JSON.parse(source.slice(start + ARCHITECTURE_CHANGE_OPEN.length, end).trim());
				const error = validateArchitectureProposalChange(change);
				return error ? { text: visibleText, change: null, error } : { text: visibleText, change, error: null };
			} catch (cause) {
				return { text: visibleText, change: null, error: `架构变更 JSON 无效：${cause instanceof Error ? cause.message : String(cause)}` };
			}
		}
		...
		function architecturePrompt({ feature, component, dashboard, message, includeContext }) {
			...
			const componentJson = component ? { id: component.id, title: component.title, kind: component.kind, containerId: component.containerId, deployment: component.deployment, status: component.status, summary: component.summary, ownedPaths: component.ownedPaths, contracts: component.contracts, dependencies: component.dependencies, supportedFeatures: component.supportedFeatures, documents: component.documents?.map((entry) => ({ level: entry.level, path: entry.path })) ?? [] } : null;
			const context = includeContext ? `
...
6. 不开始功能实现，不把 Spec 标记为 implemented，不创建或修改 .blueprint/approvals、.blueprint/architecture 或实现文件。你的机器块只是提案；只有本页的预览与确认按钮可以调用 Host 落盘。
7. Blueprint 当前没有 Component Tab、Relationships 面板或手工录入组件字段的界面。不要让开发者寻找这些入口，也不要虚构按钮、字段或关系类型。
8. 当前可执行面每次只支持一个组件的 upsert 或 delete。upsert 必须完整保留该组件全部字段：action、id、title、kind、containerId、deployment、status、summary、ownedPaths、contracts、dependencies、supportedFeatures、documents；delete 只能包含 action 与 id。
9. kind 只允许 service、library、plugin、internal、frontend、data、infra；status 只允许 planned、active、deprecated；dependencies.relation 只允许 depends_on、calls、publishes、consumes、exposes、extends。contracts 只是字符串 ID 数组；documents 只是 { level: required|recommended, path } 数组。
10. dependencies 只能指向当前目录中已经存在的 Component ID；supportedFeatures 只能使用 Feature 目录中的真实 ID。Feature 父级、Feature 自身 components 字段、Feature→Feature 关系、结构化 contract direction/purpose 当前都不能由提案卡应用；若需求涉及它们，明确标记为“当前不可执行的后续设计”，不要生成机器块。
11. 当结论已足够确定、没有必须先由开发者回答的边界问题，并且它能精确表达为一个组件变更时，在回复末尾附加且只附加一个以下格式的块。块内只能放 JSON，不要放 Markdown 代码围栏或解释：
${ARCHITECTURE_CHANGE_OPEN}
{"action":"upsert","id":"example-component","title":"Example component","kind":"internal","containerId":null,"deployment":null,"status":"planned","summary":"One explicit component responsibility.","ownedPaths":["src/example/**"],"contracts":[],"dependencies":[],"supportedFeatures":[],"documents":[{"level":"required","path":"DESIGN.md"}]}
${ARCHITECTURE_CHANGE_CLOSE}
12. 若是删除，机器块内容只能是 {"action":"delete","id":"existing-component"}。一次需要改多个组件时，先说明拆分顺序，并且本轮最多只为第一个独立变更生成机器块。

<architecture-catalog>
${JSON.stringify({ components: architecture.components.map((entry) => ({ id: entry.id, title: entry.title, kind: entry.kind, containerId: entry.containerId, deployment: entry.deployment, status: entry.status, summary: entry.summary, ownedPaths: entry.ownedPaths, contracts: entry.contracts, dependencies: entry.dependencies, supportedFeatures: entry.supportedFeatures, documents: entry.documents?.map((document) => ({ level: document.level, path: document.path })) ?? [] })) }, null, 2)}
</architecture-catalog>
...
` : "";
		...
		function architecturePreviewItems(preview) {
			const delta = preview?.delta ?? {};
			return [
				...(delta.added ?? []).map((id) => `新增组件：${id}`),
				...(delta.updated ?? []).map((id) => `更新组件：${id}`),
				...(delta.removed ?? []).map((id) => `删除组件：${id}`),
				...(delta.typedEdges ?? []).map((entry) => `${entry.removed ? "移除" : "声明"}依赖：${entry.component} ${entry.relation} → ${entry.target}`),
				...(delta.contracts ?? []).map((entry) => `${entry.action === "removed" ? "移除" : "声明"}契约：${entry.component} / ${entry.contract}`),
				...(delta.deploymentEffects ?? []).map((id) => `影响部署单元：${id}`),
				...(delta.identityEffects ?? []).map((entry) => `身份影响：${JSON.stringify(entry)}`),
				...(delta.documentEffects ?? []).map((entry) => `要求文档：${entry.path}`),
				...(delta.approvalInvalidations ?? []).map((entry) => `审批失效：${entry.file}`),
			];
		}

		function ArchitectureProposalCard({ proposal, onPreview, onApply }) {
			const [preview, setPreview] = useState(null);
			const [busy, setBusy] = useState("");
			const [error, setError] = useState("");
			const [applied, setApplied] = useState(false);
			if (proposal.error) return h("section", { className: "bp-arch-proposal invalid", role: "alert" },
				h("div", { className: "bp-arch-proposal-head" }, h("strong", null, "架构提案无法执行")),
				h("p", { className: "bp-arch-proposal-error" }, proposal.error),
			);
			const change = proposal.change;
			if (!change) return null;
			const previewItems = architecturePreviewItems(preview);
			const requestPreview = async () => {
				if (busy || applied || typeof onPreview !== "function") return;
				setBusy("preview"); setError(""); setPreview(null);
				try { setPreview(await onPreview(change)); }
				catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
				finally { setBusy(""); }
			};
			const applyPreview = async () => {
				if (!preview?.previewHash || busy || applied || typeof onApply !== "function") return;
				setBusy("apply"); setError("");
				try { await onApply(change, preview.previewHash); setApplied(true); }
				catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); setPreview(null); }
				finally { setBusy(""); }
			};
			return h("section", { className: `bp-arch-proposal${applied ? " applied" : ""}` },
				h("div", { className: "bp-arch-proposal-head" },
					h("div", null, h("strong", null, change.action === "delete" ? "组件删除提案" : "组件写入提案"), h("div", { className: "bp-arch-proposal-meta" }, `${change.action} · ${change.id}`)),
					h("span", { className: "bp-badge" }, preview ? "已预览" : "待预览"),
				),
				preview ? h("div", { className: "bp-arch-proposal-delta" },
					h("strong", null, "Host 精确变更预览"),
					previewItems.length > 0 ? h("ul", null, previewItems.map((entry, index) => h("li", { key: `${index}:${entry}` }, entry))) : h("p", { style: { margin: "4px 0 0" } }, "Host 未报告可观察差异。"),
					h("div", { className: "bp-arch-proposal-meta" }, `Preview SHA-256: ${preview.previewHash}`),
				) : null,
				error ? h("p", { className: "bp-arch-proposal-error", role: "alert" }, error) : null,
				applied ? h("p", { className: "bp-arch-proposal-success" }, "已由你确认并应用；架构图已刷新。") : h("div", { className: "bp-arch-proposal-actions" },
					h("button", { type: "button", className: "bp-button", disabled: Boolean(busy), onClick: () => void requestPreview() }, busy === "preview" ? "正在生成预览…" : preview ? "重新生成预览" : "生成变更预览"),
					preview ? h("button", { type: "button", className: "bp-button primary", disabled: Boolean(busy), onClick: () => void applyPreview() }, busy === "apply" ? "正在应用…" : "确认并应用") : null,
				),
			);
		}

		function ArchitectureAssistant({ cwd, architectureService, feature, component, dashboard, onPreviewProposal, onApplyProposal }) {
			const [sessionId, setSessionId] = useState(null);
			const [cached, setCached] = useState([]);
			const [draft, setDraft] = useState("");
			const [sending, setSending] = useState(false);
			...
			useEffect(() => {
				...
				try { rememberedSessionId = window.localStorage.getItem(sessionStorageKey); } catch {}
				const found = architectureService?.find?.(cwd, feature, component) ?? rememberedSessionId ?? null;
				setSessionId(found);
				try {
					const parsed = JSON.parse(window.localStorage.getItem(storageKey) ?? "[]");
					setCached(Array.isArray(parsed) ? parsed : []);
				...
				setError("");
				if (found) {
					try { window.localStorage.setItem(sessionStorageKey, found); } catch {}
					void architectureService?.open?.(found).catch((cause) => {
						if (!active) return;
						setSessionId(null);
						try { window.localStorage.removeItem(sessionStorageKey); } catch {}
						setError(`原架构会话暂时无法恢复；已保留本地历史，下次发送时会创建新会话。${cause instanceof Error ? ` ${cause.message}` : ""}`);
					}).finally(() => { if (active) setOpening(false); });
				}
				return () => { active = false; };
			}, [focusKey]);
			...
			const submit = async () => {
				...
				followLatest.current = true;
				setShowLatest(false);
				setSending(true); setError(""); updateDraft("");
				...
				try { window.localStorage.setItem(storageKey, JSON.stringify(optimistic)); } catch {}
				try {
					const result = await architectureService.send(cwd, feature, component, dashboard, text, sessionId);
					setSessionId(result.sessionId);
					try { window.localStorage.setItem(sessionStorageKey, result.sessionId); } catch {}
				} catch (cause) {
					setError(cause instanceof Error ? cause.message : String(cause));
				} finally {
					setSending(false);
			...
			return h("aside", { className: "bp-arch-assistant" },
				h("div", { className: "bp-arch-assistant-head" },
					h("h2", null, "架构助手"),
					h("p", null, "它基于当前架构目录、Feature 目录和已批准决策分析归属与影响；可生成提案卡，但只有你预览并确认后 Host 才会写入"),
				),
				h("div", { className: "bp-arch-assistant-head" },
				...
				h("div", { className: "bp-reviewer-scroll" },
					h("div", { ref: messagesElement, className: "bp-reviewer-messages", "aria-live": "polite", "aria-busy": snapshot?.running ? "true" : "false", onScroll: (event) => updateScrollPin(event.currentTarget) },
						messages.length === 0 ? h("div", { className: "bp-reviewer-empty" }, loadingSession ? "正在载入架构对话…" : "描述你想新增的能力或系统调整。\n架构助手会在这里持续展示分析、工具活动和结果。") : messages.map((message) => {
							if (message.type === "activity") return h("details", { key: message.key, className: "bp-review-activity", "data-status": message.status, open: message.status === "running" ? true : undefined },
								h("summary", null, h("span", { className: "bp-activity-dot", "aria-hidden": "true" }), h("span", { className: "bp-activity-title" }, message.title), h("span", { className: "bp-activity-state" }, message.status === "running" ? "进行中" : message.status === "failed" ? "失败" : "完成")),
								message.detail ? h("pre", { className: "bp-activity-detail", tabIndex: 0 }, message.detail) : null,
							);
							const proposal = message.role === "assistant" && !message.partial ? parseArchitectureProposal(message.text) : { text: message.text, change: null, error: null };
							return h("div", { key: message.key, className: `bp-review-turn ${message.role}` },
								h("div", { className: "bp-review-role" }, message.role === "user" ? "你" : "架构助手"),
								h("div", { className: `bp-review-message${message.partial ? " partial" : ""}`, tabIndex: 0 }, message.role === "assistant" ? h("div", { className: "bp-review-markdown" },
									proposal.text ? h(MarkdownText, { text: proposal.text, streaming: Boolean(message.partial), codeLabels: REVIEW_CODE_LABELS }) : null,
									message.partial ? h("span", { className: "bp-stream-cursor", "aria-label": "正在生成" }) : null,
									proposal.change || proposal.error ? h(ArchitectureProposalCard, { proposal, onPreview: onPreviewProposal, onApply: onApplyProposal }) : null,
								) : message.text),
							);
						}),
					),
				...
				h("div", { className: "bp-arch-assistant-compose" },
					h("textarea", { value: draft, disabled: sending, placeholder: "描述你想新增的能力或改造，让架构助手返回归属、分配、依赖、部署边界、备选方案和未决问题…", onChange: (event) => updateDraft(event.target.value), onKeyDown: (event) => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); void submit(); } } }),
		...
		function BlueprintView({ cwd, sendPrompt, reviewer, architectureReviewer }) {
			...
			const [architectureMounted, setArchitectureMounted] = useState(false);
			...
			useEffect(() => { setData(null); setSetup(null); setEditing(false); setDraft(null); setWorkspaceTab("spec"); setArchitectureMounted(false); setSelectedComponentId(null); setNotice(""); void load(); }, [cwd]);
			...
			const previewArchitectureProposal = async (change) => {
				setError(""); setNotice("");
				const result = await callApi({ action: "architecture-preview", cwd, change });
				return result.preview;
			};
			const applyArchitectureProposal = async (change, expectedPreviewHash) => {
				setError(""); setNotice("");
				const result = await callApi({ action: "architecture-apply", cwd, change, expectedPreviewHash });
				setData(result.dashboard);
				setSelectedComponentId((current) => change.action === "delete" && current === change.id ? null : current);
				setNotice(change.action === "delete" ? `组件 ${change.id} 已删除；架构图已刷新。` : `组件 ${change.id} 已应用；架构图已刷新。`);
				return result;
			};
			const renderArchitectureTab = () => {
				...
				return h("div", null,
					...
					h("main", { className: "bp-arch-workspace" },
						...
						architectureReviewer ? h(ArchitectureAssistant, { cwd, architectureService: architectureReviewer, feature: selected, component: selectedComponent, dashboard: data, onPreviewProposal: previewArchitectureProposal, onApplyProposal: applyArchitectureProposal }) : null,
					),
				);
		};
			...
			return h("div", { className: "bp-root" }, h("div", { className: "bp-shell" },
				...
				h("nav", { className: "bp-primary-tabs", "aria-label": "Blueprint 工作区" },
					...
					h("button", { type: "button", className: `bp-primary-tab${workspaceTab === "architecture" ? " active" : ""}`, "aria-selected": workspaceTab === "architecture", onClick: () => { setArchitectureMounted(true); setWorkspaceTab("architecture"); } }, "架构设计"),
				),
				...
				) : workspaceTab === "structure" ? h("div", null,
				...
				architectureMounted ? h("div", { hidden: workspaceTab !== "architecture", "aria-hidden": workspaceTab !== "architecture" }, renderArchitectureTab()) : null,
			));
		}
		...
		function apply(ctx) {
			...
			const architectureService = {
				...
				async send(cwd, feature, component, dashboard, message, preferredSessionId = null) {
					let sessionId = preferredSessionId && ctx.sessions.binding?.(preferredSessionId)?.session ? preferredSessionId : findArchitectureReview(cwd, feature, component);
		...
		exports.parseArchitectureProposal = parseArchitectureProposal;
