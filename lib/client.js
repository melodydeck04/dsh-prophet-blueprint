...
`;
		...
		function DocumentViewer({ cwd, feature, file, onClose, setError }) {
			const [document,setDocument]=useState(null);
			useEffect(()=>{let alive=true;setDocument(null);callApi({action:"document",cwd,featureId:feature.id,file}).then((value)=>{if(alive)setDocument(value);}).catch((error)=>setError(error.message));return()=>{alive=false;};},[cwd,feature.id,file]);
			return h("section",{className:"bp-panel"},h("div",{className:"bp-head"},h("div",null,h("h2",null,"文档详情"),h("div",{className:"bp-path"},file)),h("button",{className:"bp-button",onClick:onClose},"返回 Feature")),document?h("div",{className:"bp-doc"},h(MarkdownText,{text:document.content})):h("div",{className:"bp-empty"},"正在读取文档…"));
		}

		function FeatureDetail({ cwd, feature, features, components, reload, setNotice, setError, openDocument }) {
			...
			const approve=async()=>{if(!feature.workflow.spec||!window.confirm(`确认批准精确哈希 ${feature.workflow.spec.hash}？`))return;setBusy(true);setError("");try{await callApi({action:"approve",cwd,featureId:feature.id,expectedSpecHash:feature.workflow.spec.hash});setNotice("Spec 已批准。回到当前 DSH Chat 继续实现即可；Blueprint 不会创建第二个助手。");await reload();}catch(error){setError(error.message);}finally{setBusy(false);}};
			return h("section",{className:"bp-panel"},
				...
				h("div",{className:"bp-section"},h("h3",null,"当前行为"),h("div",{className:"bp-actions"},h("button",{className:`bp-tab${language==="zh"?" active":""}`,onClick:()=>setLanguage("zh")},"中文"),h("button",{className:`bp-tab${language==="en"?" active":""}`,onClick:()=>setLanguage("en")},"English")),brief?.exists?h("div",{className:"bp-doc"},h(MarkdownText,{text:brief.content})):h("p",{className:"bp-muted"},"当前语言的 Feature brief 尚未登记。")),
				...
				h("div",{className:"bp-section"},h("h3",null,"活动变更与历史"),feature.workflow.spec?h("div",null,h("div",{className:"bp-meta"},h("span",{className:"bp-badge"},feature.workflow.spec.status),h("span",{className:"bp-badge"},feature.workflow.internalStage)),h("p",{className:"bp-hash"},feature.workflow.spec.hash),h("button",{className:"bp-doc-button",onClick:()=>openDocument(feature.workflow.spec.file)},feature.workflow.spec.file)):h("p",{className:"bp-muted"},"当前没有 Feature-linked 变更。"),feature.workflow.internalStage==="review"&&feature.architecture?.ready?h("button",{className:"bp-button primary",disabled:busy,onClick:approve},busy?"正在批准…":"批准精确 Spec 哈希"):null),
		...
		function BlueprintView({cwd}){
			const [data,setData]=useState(null),[setup,setSetup]=useState(null),[selectedId,setSelectedId]=useState(null),[view,setView]=useState("map"),[query,setQuery]=useState(""),[stage,setStage]=useState("all"),[document,setDocument]=useState(null),[error,setError]=useState(""),[notice,setNotice]=useState(""),[busy,setBusy]=useState(false);
			const load=async()=>{if(!cwd){setError("当前 DSH Session 没有项目路径。");return;}setError("");try{const next=await callApi({action:"dashboard",cwd});setData(next);setSetup(null);setSelectedId((current)=>next.catalog.features.some((entry)=>entry.id===current)?current:next.catalog.features[0]?.id||null);}catch(cause){if(cause.code==="BLUEPRINT_PROJECT_NOT_FOUND"){try{setSetup(await callApi({action:"discover",cwd}));}catch(next){setError(next.message);}}else setError(cause.message);}};
			useEffect(()=>{setData(null);setSetup(null);setSelectedId(null);setDocument(null);void load();},[cwd]);
			const initialize=async()=>{const target=setup?.candidate?.path||setup?.manualCandidate?.path;if(!target)return;setBusy(true);try{const result=await callApi({action:"initialize",cwd,target,confirmCurrentWorkspace:!setup?.candidate});setData(result.dashboard);setSetup(null);setSelectedId(result.dashboard.catalog.features[0]?.id||null);}catch(cause){setError(cause.message);}finally{setBusy(false);}};
			if(!data)return h("div",{className:"bp-root"},h("style",null,css),h("div",{className:"bp-shell"},error?h("div",{className:"bp-alert"},error):null,setup?h("section",{className:"bp-panel"},h("h2",null,"初始化 Blueprint"),h("p",null,`目标：${setup.candidate?.path||setup.manualCandidate?.path||"未发现项目"}`),h("button",{className:"bp-button primary",disabled:busy,onClick:initialize},busy?"正在初始化…":"初始化当前项目")):h("div",{className:"bp-empty"},"正在读取 Blueprint…")));
			...
			const sidebar=h("aside",{className:"bp-panel"},
				...
				h(FeatureTree,{features:filtered,selectedId,onSelect:(id)=>{setSelectedId(id);setDocument(null);}}));
			const detail=document&&selected
				?h(DocumentViewer,{cwd,feature:selected,file:document,onClose:()=>setDocument(null),setError})
				:selected?h(FeatureDetail,{cwd,feature:selected,features:data.catalog.features,components:data.architecture.components,reload:load,setNotice,setError,openDocument:setDocument})
			...
			return h("div",{className:"bp-root"},h("style",null,css),h("div",{className:"bp-shell"},
				h("header",{className:"bp-head"},h("div",null,h("h1",{className:"bp-title"},"Blueprint",h("span",{className:"bp-version"},`v${CLIENT_VERSION}`)),h("div",{className:"bp-path"},data.project.root)),h("button",{className:"bp-button",onClick:load},"刷新")),
				error?h("div",{className:"bp-alert"},error):null,notice?h("div",{className:"bp-notice"},notice):null,
				h("nav",{className:"bp-tabs"},h("button",{className:`bp-tab${view==="map"?" active":""}`,onClick:()=>{setView("map");setDocument(null);}},"系统地图"),h("button",{className:`bp-tab${view==="audit"?" active":""}`,onClick:()=>{setView("audit");setDocument(null);}},"项目检查")),body));
		}

		function createFeatureSource(ctx){const cache=new Map(),listeners=new Map();const notify=(sessionId)=>{for(const listener of listeners.get(sessionId)||[])listener();};const warm=async(session)=>{const row=ctx.sessions.list.getSnapshot().byId[session.sessionId];if(!row?.cwd)return;try{const dashboard=await callApi({action:"dashboard",cwd:row.cwd});cache.set(session.sessionId,dashboard.catalog.features);notify(session.sessionId);}catch{cache.set(session.sessionId,[]);notify(session.sessionId);}};return{trigger:"@",name:"Feature",order:15,showGroupTitle:true,warm(session){void warm(session);},async candidates(session,{query,signal}){if(!cache.has(session.sessionId))await warm(session);if(signal.aborted)return[];const needle=String(query||"").toLowerCase();return(cache.get(session.sessionId)||[]).filter((feature)=>!needle||`${feature.id} ${feature.title} ${feature.summary}`.toLowerCase().includes(needle)).map((feature)=>({name:feature.title,description:`${feature.id} · ${feature.summary}`,value:feature.id}));},onPick({candidate}){return{insert:{source:FEATURE_SOURCE,ref:candidate.value,label:candidate.name,clipboardText:`@feature:${candidate.value}`}};},lexicon(session){return cache.get(session.sessionId)?.map((feature)=>`feature:${feature.id}`);},subscribeLexicon(session,listener){const rows=listeners.get(session.sessionId)||new Set();rows.add(listener);listeners.set(session.sessionId,rows);return()=>rows.delete(listener);},codec:{clipboardText:(ref)=>`@feature:${ref}`,async serialize(ref){return`@feature:${ref}`;}}};}

		const inject=["slots","sessions","inputTriggers"];
		function apply(ctx){ctx.inputTriggers.registerSource(createFeatureSource(ctx));ctx.slots.inject("conversation.view",()=>ctx.slots.register({name:"conversation.view",id:"blueprint",order:20,label:()=>"Blueprint",inject:(sessionId)=>({cwd:ctx.sessions.list.getSnapshot().byId[sessionId]?.cwd||""})},BlueprintView));}
		exports.apply=apply;exports.inject=inject;exports.callApi=callApi;exports.commandHint=commandHint;exports.createFeatureSource=createFeatureSource;
