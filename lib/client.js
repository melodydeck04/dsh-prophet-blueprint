...
`;
		...
		// DSH Web 0.1.1-rc.2's MarkdownText component throws "Cannot read
		// properties of undefined (reading 'code')" inside its useMemo
		// (index-Df-65__b.js:84) when a code fence in the source text is in
		// a state the streaming parser does not expect. Until upstream fixes
		// the parser, render markdown inside a local error boundary that
		// degrades to a readable <pre> block on failure — keeping the rest
		// of BlueprintView alive.
		class MarkdownBoundary extends React.Component {
			constructor(props) { super(props); this.state = { error: null }; }
			static getDerivedStateFromError(error) { return { error }; }
			componentDidCatch(error, info) {
				console.warn("[design-blueprint] DSH MarkdownText threw, falling back to <pre>:", error?.message || error);
			}
			render() {
				if (this.state.error) {
					return h("pre", { className: "bp-fallback" }, this.props.text || "");
				}
				return this.props.children;
			}
		}
		function SafeMarkdownText(props) {
			return h(MarkdownBoundary, { text: props.text }, h(MarkdownText, props));
		}
		function DocumentViewer({ cwd, sessionId, dshWorkspacePath, dshWorkspaceTitle, feature, file, onClose, setError }) {
			...
			return h("section",{className:"bp-panel"},h("div",{className:"bp-head"},h("div",null,h("h2",null,"文档详情"),h("div",{className:"bp-path"},file)),h("button",{className:"bp-button",onClick:onClose},"返回 Feature")),document?h("div",{className:"bp-doc"},h(SafeMarkdownText,{text:document.content})):h("div",{className:"bp-empty"},"正在读取文档…"));
		}
		...
		function FeatureDetail({ cwd, sessionId, dshWorkspacePath, dshWorkspaceTitle, feature, features, components, reload, setNotice, setError, openDocument }) {
			...
			return h("section",{className:"bp-panel"},
				...
				h("div",{className:"bp-section"},h("h3",null,"当前行为"),h("div",{className:"bp-actions"},h("button",{className:`bp-tab${language==="zh"?" active":""}`,onClick:()=>setLanguage("zh")},"中文"),h("button",{className:`bp-tab${language==="en"?" active":""}`,onClick:()=>setLanguage("en")},"English")),brief?.exists?h("div",{className:"bp-doc"},h(SafeMarkdownText,{text:brief.content})):h("p",{className:"bp-muted"},"当前语言的 Feature brief 尚未登记。")),
		...
		function BlueprintView({cwd,sessionId,dshWorkspacePath,dshWorkspaceTitle}){
			...
			useEffect(()=>{setData(null);setSetup(null);setSelectedId(null);setDocument(null);void load().catch((err)=>console.warn("[design-blueprint] load rejected:",err?.message||err));},[cwd,sessionId,dshWorkspacePath,dshWorkspaceTitle]);
			...
			return h("div",{className:"bp-root"},
				...
				h("div",{className:"bp-shell"},
					...
					h("nav",{className:"bp-tabs"},
						h("button",{className:`bp-tab${view==="map"?" active":""}`,onClick:()=>{setView("map");setDocument(null);}},"系统地图"),
						h("button",{className:`bp-tab${view==="audit"?" active":""}`,onClick:()=>{setView("audit");setDocument(null);}},"项目检查")
					),
					body
				)
			);
