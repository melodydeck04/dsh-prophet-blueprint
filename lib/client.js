...
`;
		...
		function FeatureDetail({ cwd, sessionId, dshWorkspacePath, dshWorkspaceTitle, feature, features, components, reload, setNotice, setError, openDocument }) {
		...
		function sessionContext(ctx,sessionId){
			...
			return{sessionId,cwd:row?.cwd||row?.identity?.cwd||"",dshWorkspacePath:row?.dshWorkspacePath||row?.workspacePath||workspace?.path||"",dshWorkspaceTitle:row?.dshWorkspaceTitle||row?.workspaceTitle||workspace?.title||""};
		}
		function BlueprintView({cwd,sessionId,dshWorkspacePath,dshWorkspaceTitle}){
			const [data,setData]=useState(null),[setup,setSetup]=useState(null),[selectedId,setSelectedId]=useState(null),[view,setView]=useState("map"),[query,setQuery]=useState(""),[stage,setStage]=useState("all"),[document,setDocument]=useState(null),[error,setError]=useState(""),[notice,setNotice]=useState(""),[busy,setBusy]=useState(false),[projectPath,setProjectPath]=useState("");const requestContext=()=>({cwd,sessionId,dshWorkspacePath,dshWorkspaceTitle});
			const load=async()=>{if(!cwd&&!sessionId&&!dshWorkspacePath){setError("当前 DSH Session 没有项目或 workspace 线索。");return;}setError("");try{const next=await callApi({action:"dashboard",...requestContext()});setData(next);setSetup(null);setSelectedId((current)=>next.catalog.features.some((entry)=>entry.id===current)?current:next.catalog.features[0]?.id||null);}catch(cause){if(cause.code==="BLUEPRINT_PROJECT_NOT_FOUND"){try{setSetup(await callApi({action:"discover",...requestContext()}));}catch(next){setError(next.message);}}else setError(cause.message);}};
			useEffect(()=>{setData(null);setSetup(null);setSelectedId(null);setDocument(null);void load();},[cwd,sessionId,dshWorkspacePath,dshWorkspaceTitle]);
			const initialize=async()=>{const target=setup?.candidate?.path||setup?.manualCandidate?.path;if(!target)return;setBusy(true);try{const result=await callApi({action:"initialize",...requestContext(),target,confirmCurrentWorkspace:!setup?.candidate});setData(result.dashboard);setSetup(null);setSelectedId(result.dashboard.catalog.features[0]?.id||null);}catch(cause){setError(cause.message);}finally{setBusy(false);}};const bindProject=async()=>{const target=projectPath.trim();if(!target){setError("请输入 Blueprint 项目路径。");return;}setBusy(true);setError("");try{const next=await callApi({action:"bind",...requestContext(),target});setData(next);setSetup(null);setSelectedId(next.catalog.features[0]?.id||null);setNotice(`已设置兜底项目：${next.project.root}`);}catch(cause){setError(cause.message);}finally{setBusy(false);}};
			...
			const detail=document&&selected
				?h(DocumentViewer,{cwd,sessionId,dshWorkspacePath,dshWorkspaceTitle,feature:selected,file:document,onClose:()=>setDocument(null),setError})
				:selected?h(FeatureDetail,{cwd,sessionId,dshWorkspacePath,dshWorkspaceTitle,feature:selected,features:data.catalog.features,components:data.architecture.components,reload:load,setNotice,setError,openDocument:setDocument})
			...
			return h("div",{className:"bp-root"},h("style",null,css),h("div",{className:"bp-shell"},
				h("header",{className:"bp-head"},h("div",null,h("h1",{className:"bp-title"},"Blueprint",h("span",{className:"bp-version"},`v${CLIENT_VERSION}`)),h("div",{className:"bp-path"},data.project.workspace?h("div",null,`DSH workspace · ${data.project.workspace.title||data.project.workspace.path}`):null,h("div",null,`Session cwd · ${data.project.sessionCwd||"未暴露"}`),h("div",null,`Blueprint root · ${data.project.root}`),data.project.bound?h("span",{className:"bp-version"},"manual fallback"):null)),h("button",{className:"bp-button",onClick:load},"刷新")),
		...
		function apply(ctx){ctx.inputTriggers.registerSource(createFeatureSource(ctx));ctx.slots.inject("conversation.view",()=>ctx.slots.register({name:"conversation.view",id:"blueprint",order:20,label:()=>"Blueprint",inject:(sessionId)=>sessionContext(ctx,sessionId)},BlueprintView));}
