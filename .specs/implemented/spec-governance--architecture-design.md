...
Primary workspace navigation must not be a conversation lifecycle boundary. After its first render, the architecture workspace remains mounted while another Blueprint tab is visible, preserving the current Session subscription, streaming state, cached turns, proposal-card state, scroll position, and unsent draft. The draft and backing Session id are also stored under the project-, protocol-, and focus-specific browser key: remount or page recovery restores the draft and tries the exact archived Session id before ordinary title discovery, while a missing stale binding may fall back to creating a new backing Session without discarding cached visible history.
...
- AC-ARCH-14: Switching from Architecture design to another primary Blueprint tab and back preserves the mounted assistant, current Session subscription and streaming/proposal state; unsent text and the exact archived backing Session id are persisted per project, protocol, and focus and are restored on remount before ordinary Session discovery.
...
- AC-ARCH-14: `tests/architecture-reviewer.test.js`, `tests/client.test.js`; always-mounted architecture workspace, hidden-tab presentation, focus-scoped draft persistence, persisted archived Session recovery, and preferred-session submission
