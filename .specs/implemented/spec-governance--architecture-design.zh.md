...
一级工作区导航不能成为对话生命周期边界。架构工作区首次渲染后，即使当前显示另一个 Blueprint Tab，也必须保持挂载，从而保留当前 Session 订阅、流式状态、缓存消息、提案卡状态、滚动位置和未发送草稿。草稿与后台 Session ID 还要按照项目、协议和聚焦目标写入浏览器键：重新挂载或页面恢复时，先恢复草稿并尝试精确的已归档 Session ID，再进行普通标题发现；若旧绑定确实不存在，可以创建新的后台 Session，但不得丢弃已经缓存的可见历史。
...
助手首先生成结构化的变更前后提案。当提案能够精确映射为一个已登记组件的新增、更新或删除时，已完成的助手消息可以携带一个经过分隔、仅使用当前组件 Schema 的机器可读变更。内嵌工作区会从对话 Markdown 中移除该机器块，并渲染提案卡片：先让 Host 计算精确架构预览，展示组件、边、部署、契约、文档和审批失效差异，然后才显示一个绑定该预览哈希、由开发者明确触发的应用操作。无效、过期、多组件或当前模型不支持的提案都不能应用。Feature 层级编辑、Feature 对 Feature 的关系类型、结构化契约元数据，以及 Blueprint 尚未实现的界面，都不得被虚构成可执行操作。

只有开发者的明确操作可以把精确的当前提案应用到已登记架构记录。助手本身仍不能写入架构文件、同步 Feature 规划文档、编辑实现文件、把生命周期文档移动到 implemented、写入审批记录或自行批准结果。后续任何 Spec 变化仍会通过现有精确哈希工作流使旧审批失效。
...
- AC-ARCH-13：已完成的架构助手回复最多携带一个完全符合当前 Schema 的组件变更；Blueprint 把它转换为内嵌提案卡片，先获取并展示 Host 计算的预览，再启用一个使用精确预览哈希、由开发者确认的独立应用操作，成功后刷新架构图；格式错误、过期、多组件或不支持的动作必须被拒绝，而且不得虚构不存在的管理 Tab 或关系类型。
- AC-ARCH-14：从“架构设计”切换到另一个 Blueprint 一级 Tab 再返回时，必须保留已挂载的助手、当前 Session 订阅及流式/提案状态；未发送文本和精确的已归档后台 Session ID 要按照项目、协议和聚焦目标持久化，并在重新挂载时优先于普通 Session 发现恢复。
...
- AC-ARCH-13：`tests/architecture-reviewer.test.js`、`tests/client.test.js`、`tests/web-api.test.js`；分隔提案解析、当前 Schema 提示契约、内嵌预览/应用卡片、精确哈希 Host 桥接、无效提案拒绝和仪表盘刷新
- AC-ARCH-14：`tests/architecture-reviewer.test.js`、`tests/client.test.js`；架构工作区常驻挂载、非当前 Tab 隐藏展示、按聚焦目标持久化草稿、已归档 Session ID 恢复和优先 Session 提交
