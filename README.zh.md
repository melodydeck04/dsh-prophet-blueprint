...
组件目录为空时，可以在逻辑组件图中点击**初始化架构模型**。Blueprint 会根据当前包/项目身份、发布文件边界和 `DESIGN.md` 生成一个仓库级起始组件，向你展示其 ID、类型和拥有路径，确认后再通过同一套哈希绑定架构预览流程应用。初始化不会覆盖或追加到非空目录，并且只建立一个可继续拆分的粗粒度边界，不会假装已经推断出全部内部组件。
...
“架构设计”第一次打开后，切换到其他 Blueprint Tab 只会隐藏工作区，不会将其卸载。因此返回时，当前 Session 订阅、流式回复、提案卡状态、滚动位置和未发送草稿都会保持。未发送草稿和精确的已归档后台 Session ID 还会按照当前项目、架构协议及选中的 Feature/组件聚焦目标持久化。重新挂载时会先恢复这些值并尝试精确 Session，再按标题查找；如果旧绑定已经不可用，可见历史缓存仍会保留，并在下次提交时创建替代后台 Session。
...
助手首先生成结构化的变更前后提案。当结果能够精确表达为一个符合当前 Schema 的组件新增、完整更新或删除时，已完成的回复会直接变成内嵌提案卡片，而不是给出手工改文件的说明。点击**生成变更预览**后，Host 会计算精确的组件、类型化边、部署、契约、文档和审批失效差异；只有预览成功后才会显示**确认并应用**。确认操作会使用同一个预览 SHA-256 应用变更并刷新架构图，预览过期或引用无效时会失败关闭。

这个卡片不会虚构 Component Tab 或 Relationships 面板。它一次只处理一个已登记组件变更，并且只支持当前真实的种类、状态、字符串契约 ID、文档声明和 Component 对 Component 关系类型。Feature 父级编辑、Feature 对 Feature 关系、带 direction/purpose 的结构化契约元数据、多组件事务、功能说明变化和 proposed Spec 变化仍属于讨论结果或另一条受治理工作流。架构 Session 本身不能写入、编辑实现文件、把生命周期文档移动到 implemented、写入审批记录或自行批准结果。
